...
public partial class SupervisorLocationService : NSObject, ICLLocationManagerDelegate
{
    ...
    private bool isFingerprinting = false;
    ...
    public void LocationsUpdated(CLLocationManager manager, CLLocation[] locations)
    {
        try
        {
            ...
            if (location != null)
            {
                if (isFingerprinting)
                {
                    if (LocationCount > Constants.FpTotalReadings / 2)
                    {
                        StopFingerprint();
                        return;
                    }
                ...
                PersistLocation(location, ELocationClass.A, isFingerprinting);
            }
        }
        catch (Exception ex)
        {
    ...
    public void PersistLocation(CLLocation location, ELocationClass locationClass, bool isFingerprint = false)
    {
        if (AppServices.Current.SupervisorTrackerService.SupervisorVisit != null && AppServices.Current.SupervisorTrackerService.SupervisorVisit.Id > 0 && !isFingerprinting)
        {
            ...
            _ = AppServices.Current.SupervisorTrackerService.ProcessLocation(dbLocation);
        }
        ...
        if (AppServices.Current.FingerprintService.ServiceUserFp != null)
        {
            ...
            _ = AppServices.Current.FingerprintService.ProcessLocation(dbLocation);
        }
    }

    public void PersistStepCount(VisitStepCount stepCount)
    {
        _ = AppServices.Current.SupervisorTrackerService.ProcessStepCount(stepCount);
    }
    ...
    public partial void StartFingerprint()
    {
        ...
        _ = AppServices.Current.FingerprintService.UpdateFingerprint(EPlatform.iOS.ToString());
    }
